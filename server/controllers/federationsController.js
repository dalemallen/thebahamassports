...
const getAllFederations = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federations WHERE deleted_at IS NULL"
	);
	res.json(result.rows);
};

const getFederationById = async (req, res) => {
	const result = await pool.query("SELECT * FROM federations WHERE id = $1", [
		req.params.id,
	]);
	res.json(result.rows[0]);
};

// controllers/federationsController.js

const getFederationByUser = async (req, res) => {
	const auth0Id = req.user.sub; // still in auth0 format

	try {
		// Step 1: Get internal user UUID from auth0_id
		const userResult = await pool.query(
			`SELECT id, federation_id FROM users WHERE auth0_id = $1`,
			[auth0Id]
		);
		...
		const { federation_id } = userResult.rows[0];

		if (!federation_id) {
			return res
				.status(404)
				.json({ error: "User not linked to any federation" });
		}

		// Step 2: Fetch federation by ID
		const federationResult = await pool.query(
			`SELECT * FROM federations WHERE id = $1`,
			[federation_id]
		);

		if (federationResult.rowCount === 0) {
			return res.status(404).json({ error: "Federation not found" });
		}

		res.json(federationResult.rows[0]);
	} catch (err) {
...
const getFederationBySport = async (req, res) => {
	...
	try {
		...
		if (result.rows.length === 0) {
			return res
				.status(404)
				.json({ message: "Federation not found for this sport." });
		}
		res.json(result.rows[0]);
	} catch (err) {
...
const updateFederation = async (req, res) => {
	...
	await pool.query(`UPDATE federations SET ${setClause} WHERE id = $1`, [
		req.params.id,
		...values,
	]);
...
const deleteFederation = async (req, res) => {
	await pool.query("UPDATE federations SET deleted_at = NOW() WHERE id = $1", [
		req.params.id,
	]);
...
const getFederationRoles = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_roles WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationEvents = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM events WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationNews = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_news WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const deleteFederationDocument = async (req, res) => {
	await pool.query(
		"DELETE FROM federation_documents WHERE id = $1 AND federation_id = $2",
...
const getFederationNotes = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_notes WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationMessages = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_messages WHERE federation_id = $1 ORDER BY created_at DESC",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationEquipment = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_equipment WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationDues = async (req, res) => {
	const result = await pool.query(
		"SELECT * FROM federation_dues WHERE federation_id = $1",
		[req.params.id]
	);
	res.json(result.rows);
};
...
const getFederationAwards = async (req, res) => {
	...
	try {
		const result = await pool.query(
			"SELECT * FROM federation_awards WHERE federation_id = $1",
			[id]
		);
		res.json(result.rows);
	} catch (err) {
...
const getFederationSettings = async (req, res) => {
	...
	try {
		const result = await pool.query(
			"SELECT * FROM federation_settings WHERE federation_id = $1",
			[id]
		);
		res.json(result.rows[0] || {});
	} catch (err) {
...
// server/controllers/federationsController.js

const completeFederationOnboarding = async (req, res) => {
	try {
		const userId = req.user?.id; // This should be a UUID
		const { federation_id, ...fields } = req.body;

		if (!userId || !federation_id) {
			return res
				.status(400)
				.json({ error: "Missing user ID or federation ID" });
		}

		await pool.query(`UPDATE federations SET user_id = $1, ... WHERE id = $2`, [
			userId,
			federation_id,
		]);
		...
		res.json({ message: "Federation onboarding completed" });
	} catch (err) {
		console.error("Onboarding error", err);
		res
			.status(500)
			.json({ error: "Server error during federation onboarding" });
	}
};
...
const exportFederationSummary = async (req, res) => {
	try {
		...
		const [teamsRes, athletesRes] = await Promise.all([
			pool.query("SELECT id, name FROM teams WHERE federation_id = $1", [
				federationId,
			]),
			pool.query(
				`
      ...
      `,
		...
		const csv = new Parser({
			fields: ["first_name", "last_name", "position", "club_team"],
		}).parse(athletesRes.rows);
		res
			.header("Content-Type", "text/csv")
			.attachment("federation_summary.csv")
			.send(csv);
	} catch (err) {
